-- ============================================
-- FIX 4: search_services() — gjenstående korrelert subquery i scored_services
--
-- sql/34 (_fix_search_late_projection.sql) løste den dyreste korrelasjonen:
-- `reasons`-subqueryen ble flyttet til etter LIMIT via limited_services-CTEen.
-- Funksjonen reduserte kjøretid vesentlig, men kommentaren i sql/34 («INGEN
-- korrelerte subqueries her — kun ren aritmetikk/boolske uttrykk») var
-- FEIL: én korrelert EXISTS-subquery ble stående igjen i scored_services:
--
...
--   )) AS type_match
--
-- Denne kjøres for ALLE ~3 500 kandidater som overlever coverage-matching
-- — ikke bare de max_limit (50) som faktisk returneres. Hvert kall er raskt
-- (~0,5 ms via UNIQUE(service_id, type)-indeksen), men 3 500 × 0,5 ms =
-- ~1,75 s — lagt oppå resten av funksjonens arbeid.
--
-- Samlet API-tid etter sql/34: ~3,2–4 s (bekreftet i sql/34-kommentaren).
-- Dersom Supabase sin statement_timeout for anon/authenticated er ≤ 3 000 ms,
-- vil kall med spesifikk servicetype-filter (type_candidate != 'any') mot
-- store kandidatsett (tett by, bred radius) sporadisk trygge → HTTP 500.
-- Dette forklarer de 16 intermittente 500-feilene i logganalysen.
--
-- FIX: erstatt EXISTS-subqueryen med en MATERIALIZED CTE + LEFT JOIN.
--
--   type_matched_services AS MATERIALIZED (
--     SELECT service_id FROM service_types
--     WHERE NOT (type_candidate IN ('any', ''))
--       AND type = type_candidate
--   )
--
-- Effekt:
--   - Når type_candidate = 'any' (flertallet av kall): CTE er tom med én gang
--     (WHERE-betingelsen er false) → LEFT JOIN gir NULL → type_match = TRUE
--     for alle rader. Overhead: tilnærmet null.
--   - Når type_candidate = 'styrke' o.l.: CTE bygges ÉN gang via
--     idx_service_types_type-indeksen → PostgreSQL gjør deretter én hash-join
--     mot kandidatsettet i stedet for ~3 500 individuelle correlated lookups.
--
-- ETTER DEPLOY — kjør dette i Supabase SQL Editor for å bekrefte fiksen:
--
--   EXPLAIN (ANALYZE, BUFFERS)
--   SELECT * FROM search_services(
--     'oslo', 59.9139, 10.7522,
--     'any', 'styrke', 'any', NULL, 'nearest', NULL,
--     50, NULL, NULL, NULL, NULL, NULL, 0
--   );
--
-- Ønsket resultat: "Materialize" eller "Hash Join" på service_types-settet
-- EN gang tidlig i planen. Uønsket: "Index Scan on service_types" gjentatt
-- per rad i scored_services. Verifiser at ingen "Function Scan" med 3 500+
-- rows vises for service_types inne i scored_services.
--
-- Ingen endring i parameterliste (16 params), RETURNS TABLE (26 kolonner),
-- hvilke rader som returneres eller sorteringsrekkefølge.
-- ============================================
...
BEGIN
  ...
  -- NY CTE: forhåndshenter alle service_id-er med ønsket type — ÉN gang, som
  -- en materialisert mengde. LEFT JOIN i scored_services under bruker denne.
  -- Når type_candidate = 'any': WHERE er false → tom CTE → JOIN gir NULL →
  -- type_match = TRUE for alle rader. Overhead: tilnærmet null.
  -- Når type_candidate = 'styrke': bruker idx_service_types_type til å hente
  -- alle kvalifiserte service_id-er i ett hash-oppslag i stedet for ~3 500
  -- individuelle correlated lookups.
  type_matched_services AS MATERIALIZED (
    SELECT service_id
    FROM service_types
    WHERE NOT (type_candidate IN ('any', ''))
  ...
  -- LAG (1/2): match-flagg + score for ALLE kandidater som overlevde
  ...
  -- Etter fiksen i denne migrasjonen: INGEN korrelerte subqueries her —
  -- kun ren aritmetikk, boolske uttrykk og én hash-join mot type_matched_services.
  scored_services AS (
    SELECT
      rs.*,
      (
    ...
    FROM (
      ...
      FROM (
        SELECT rs0.*,
          ...
          -- ENDRET: var EXISTS(...) — korrelert subquery O(kandidater). Nå: LEFT JOIN + IS NOT NULL.
          (type_candidate IN ('any', '') OR te.service_id IS NOT NULL) AS type_match,
        ...
        LEFT JOIN type_matched_services te ON te.service_id = rs0.id
      ) AS matched
    ) AS rs
  ...
  -- LAG (2/2): sortering + trimming til maks max_limit rader HER, FØR
  -- den ytterste SELECT-en under bygger reasons/match_reason/lat/lon. Dette
  -- er fiksen fra sql/34 som fortsatt gjelder uendret.
  limited_services AS (
    SELECT ss.*
  ...
  -- hele kandidatsettet — dette er fiksen fra sql/34 som fortsatt gjelder uendret.
  SELECT
    ls.id,
    ls.name,
    ls.type,
