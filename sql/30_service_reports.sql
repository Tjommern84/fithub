-- ============================================
-- "Rapporter feil"-flagg — services.reported_at + service_reports-tabell
--
-- En anonym besøkende kan rapportere en tjeneste som feil/nedlagt/duplikat.
-- Første rapport setter services.reported_at, og search_services() filtrerer
-- umiddelbart vekk raden fra søketreff (se gotcha i CLAUDE.md — bevisst
-- forenkling for denne runden, ingen terskel/admin-godkjenning, må revurderes
-- før launch pga. misbrukspotensial mot konkurrenters oppføringer).
--
-- search_services(): input-parameterlisten er UENDRET (samme 16 params som
-- sql/29). RETURNS TABLE er også uendret. DROP+CREATE kjøres likevel som
-- forsiktighetsregel siden hele funksjonskroppen erstattes (jf. mønsteret i
-- sql/29s kommentar).
--
-- Full funksjonskropp kopiert fra sql/29_search_services_provider_type.sql
-- (siste autoritative versjon, jf. CLAUDE.md — IKKE 01_postgis_and_search.sql),
-- med ett tillegg: `AND s.reported_at IS NULL` i WHERE-klausulen, ved siden
-- av `s.is_active = true`. Ingen annen logikk endret.
-- ============================================

ALTER TABLE services ADD COLUMN IF NOT EXISTS reported_at timestamptz DEFAULT NULL;
...
DROP POLICY IF EXISTS service_reports_insert_anon ON service_reports;
CREATE POLICY service_reports_insert_anon ON service_reports
  FOR INSERT TO anon
  WITH CHECK (true);

-- Ingen SELECT-policy for anon/authenticated — kun service_role (admin) leser
-- denne tabellen, til fremtidig adminpanel-bruk.
