-- ============================================
-- search_services() — legg til provider_type i returkolonnene
--
-- Input-parameterlisten er UENDRET (samme 16 params som sql/23). Likevel
-- kreves DROP FUNCTION FØR CREATE OR REPLACE her: Postgres tillater ikke
-- CREATE OR REPLACE når RETURNS TABLE-kolonnelisten endres (egen feilmelding,
-- "cannot change return type of existing function"), uavhengig av
-- parameter-overload-gotchaen fra sql/23. Samme forsiktighetsregel, ny grunn.
--
-- Full funksjonskropp kopiert fra sql/23_fix_search_services_overload.sql
-- (siste autoritative versjon, jf. CLAUDE.md — IKKE 01_postgis_and_search.sql),
-- med ÉN tillegg: provider_type lagt til i RETURNS TABLE og SELECT-listen.
-- Ingen annen logikk endret.
-- ============================================
